...
class Config:
    ...
    ### YOUR CODE HERE
    n_window_features = (1+2*window_size)*n_word_features # The total number of features used for each window.
    ### END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    windowed_data = []
    start_tokens = [start] * window_size
    end_tokens = [end] * window_size
    for sentence, labels in data:
        padded_sentence = start_tokens + sentence + end_tokens
        for idx, word in enumerate(sentence):
            window = padded_sentence[idx:(idx + 2 * window_size + 1)]
            window_flat = [feature for word in window for feature in word]
            window_label_pair = (window_flat, labels[idx])
            windowed_data.append(window_label_pair)
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        emb_lookup = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder)
        embeddings = tf.reshape(emb_lookup, shape=(-1, self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_loss_op(self, pred):
        ...
        loss_per_datum = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = pred, labels = self.labels_placeholder)
        loss = tf.reduce_mean(loss_per_datum)
        ### END YOUR CODE
        return loss
